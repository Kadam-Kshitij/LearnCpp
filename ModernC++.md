...
| std::multimap< int, std::string, std::greater< int > > | No | Yes | empty, size, swap, count, find | clear, insert, emplace, erase |  Contains key-value pairs. Keys are sorted and can be repeated. Search, removal, and insertion operations have logarithmic complexity. |
